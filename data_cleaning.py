...
def clean_feature_data():
    ...
    # todo: this was only hotfix,
    #  write this in a general way through regex, not statically
    ...
    # todo: would this block be possible with pandas only?
    towns = []
    is_foreign = []
    ...
    # Put together total into one column prices and prices after sale.
...
def aggregate_feature_data():
    """
    Aggregate feature data into the form that can be used
    for plotting
    ...
    for i in range(3):
        for type_ in c.TYPES_FLAT + c.TYPES_HOUSE + c.TYPES_LAND:
            ...
            if not i:
                new_df[f"{c.SHORT_PRAGUE} {type_}"] = [price]
            elif i == 1:
                new_df[f"{c.SHORT_TOP9} {type_}"] = [price]
            else:
                new_df[f"{c.SHORT_REST} {type_}"] = [price]
    new_df[c.PERIOD] = [datetime.date.today()]
